...
{
    /// <summary>
    /// The box an NPC talks to you from, drawn where the games draw it and, when the ROM's own font can
    /// be read, written with the letters the game itself would use.
    ///
    /// The map is shown at whatever size the window happens to be, so a DS screen is fitted inside the
    /// view first and everything is placed on that, which keeps the box the same shape and in the same
    /// spot however the window is resized.
    /// </summary>
    public sealed class FieldMessageBoxView : Control
    {
        ...
        /// <summary>
        /// The game's own font, read out of the loaded ROM. Null falls back to ordinary type, which is
        /// what happens when there is no ROM open or its font archive cannot be read.
        /// </summary>
        ...
        /// <summary>
        /// The border the games draw round the box, read out of the ROM. There are twenty to pick
        /// between and the player chooses in Options, so which one is right is a matter of taste.
        /// </summary>
        ...
        /// <summary>
        /// How wide a run of letters is, in DS pixels. The window hands this to the view model so the
        /// wrapping is worked out with the same measurements the drawing uses.
        /// </summary>
        ...
        /// <summary>
        /// Fits just the box to the control rather than the whole DS screen. The preview over the map
        /// wants the screen, so the box sits where it really does; a pane that only shows the box wants
        /// this, so it is not mostly empty space.
        /// </summary>
        public bool FitToFrame { get => GetValue(FitToFrameProperty); set => SetValue(FitToFrameProperty, value); }

        /// <summary>
        /// Where the DS screen lands inside the control: the top left corner in control coordinates and
        /// how much everything is blown up by.
        /// </summary>
