...
{
    /// <summary>
    /// A map running on its own: the water moves and the people turn and wander the way their movement
    /// code says they should. Look only, nothing here changes the ROM, and scripts are not run.
    /// </summary>
    public partial class AnimatedPreviewWindow : Window
    {
        ...
        public AnimatedPreviewWindow()
        {
            ...
            // Catch the keys on the way down rather than on the way back up. A focused button or tick
            // box swallows Enter before the window ever sees it, which left stepping in unable to talk
            // to anybody unless you happened to click the map first.
        ...
        /// <summary>
        /// Shows the scene the editor already has on screen. The foot finder says where an overworld
        /// stands in the scene, which the editor already works out for its own markers.
        /// </summary>
        ...
        /// <summary>
        /// Back to the editor's own view when the player stops walking about. The field camera is much
        /// narrower than the editor one, so leaving it set would make looking around the map feel wrong.
        /// </summary>
        ...
        /// <summary>
        /// Plays what a script asked for. A sound effect, a fanfare and the music are all sequences in the
        /// ROM's own sound archive, so they go through the same renderer. A cry is built differently but
