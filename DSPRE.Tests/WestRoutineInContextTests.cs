...
{
    /// <summary>
    /// The routines that need the rest of their script around them.
    ///
    /// Seven routines act on something an earlier command creates: a particle emitter, a dropped copy of
    /// the sprite, or a cell actor. Calling one on its own gives it nothing to act on, so the one-command
    /// harness in <see cref="WestRoutineEffectTests"/> could not tell a working routine from an ignored
    /// one and said so. This runs the whole script that calls it, then runs the same script again with
    /// that one call pointed at TEST_1, which the games leave empty (wsp_sample.c), and requires the two
    /// runs to come out different.
    ///
    /// Two things are needed to make that mean anything. Scripts branch on which side is attacking
    /// (TURN_CHK), and plenty of these calls sit in only one of the two halves, so every script is run
    /// from both sides; a call in the half a run skipped proves nothing either way. And a run only counts
    /// once the player says it reached the routine, so "no difference" cannot be an unreached command in
    /// disguise.
    ///
    /// What it watches is everything visible: both Pokemon, the dropped copies, the cell actors, the
    /// screen effects, and where every live particle is. It still cannot tell whether a routine does the
    /// RIGHT thing, only that the player does not drop it.
    /// </summary>
    [Collection("rom")]
