...
{
    /// <summary>
    /// The second animation a move can hold.
    ///
    /// TURN_CHK jumps from the first offset when the battle's turn count is even and from the second when
    /// it is odd (we_sys.c:3018), so a move using it has two animations and only one of them was ever
    /// visible in the editor. This sweeps every script in both games, finds the ones that branch that way,
    /// and requires each of them to actually run different commands under the two settings.
    ///
    /// Comparing the commands each run EXECUTED is what makes this able to fail: if the setting were
    /// ignored the two lists would be identical. Moves whose two offsets point at the same command are
    /// counted separately, since there is nothing there to switch between. It says nothing about whether
    /// the second animation is drawn correctly, only that it is a different one.
    /// </summary>
    [Collection("rom")]
    public class WestTurnVariantTests
    {
        ...
        private void Sweep(string project, string gameCode, WazaSeqVersion version)
        {
            ...
            foreach (var f in RomFiles.Settled(dir))
            {
                ...
                // Where the two offsets actually land. When both point at the same word the move has one
                // animation written twice over and there is nothing for the setting to switch between, so
                // that is recorded rather than counted as a failure.
