...
namespace DSPRE.Tests
{
    ...
    ///
    /// The position offset is a running total that nothing clears between frames, so an effect has to add
    /// the STEP it makes each frame. Three of them added the whole position instead, which piled up:
    /// Megahorn's charge should carry the attacker 40px and bring it back, and instead it walked 480px off
    /// the side of a 256px screen and stayed there for the rest of the move.
    ///
    /// This drives all 501 moves in both games. What it ASSERTS is that nothing travels further than the
    /// screen is wide, which is the fault above and which it would have caught. Where a sprite sits when
    /// the script ends is reported but not asserted: two moves end with their slide back home unfinished
    /// because the script stops first, and the battle engine puts sprites back between actions anyway, so
    /// demanding it here would be demanding something the games do not.
    /// </summary>
    [Collection("rom")]
