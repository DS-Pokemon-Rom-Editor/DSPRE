...
{
    /// <summary>
    /// Every particle a move uses must be drawn as itself.
    ///
    /// The preview decodes each emitter's own texture and falls back to a plain round dot when the decode
    /// fails, without saying so. On screen that is a big pale blob where the game draws a droplet or a
    /// spark, and it is why ExtremeSpeed shows two round blobs on the platform instead of a small flash.
    ///
    /// This reads every particle archive in the game, not a few, and counts the textures that do not decode,
    /// grouped by the format they are stored in.
    /// </summary>
    [Collection("rom")]
