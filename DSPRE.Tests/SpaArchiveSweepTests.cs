...
{
    /// <summary>
    /// Every particle archive in both games, read with our own parser.
    ///
    /// An emitter is a fixed block followed by whichever optional blocks its flags switch on. If any of
    /// those sizes were wrong the reader would drift and the emitters would stop somewhere other than the
    /// start of the textures, so requiring it to land exactly there is a check that can actually fail.
    /// One archive would prove nothing: a wrong size only bites where that flag is set, and some flags
    /// are set in a handful of archives out of hundreds.
    /// </summary>
    [Collection("rom")]
    public class SpaArchiveSweepTests
    {
        ...
        // The field-coverage check used to live here and found the fields with a pattern over the
        // source, which matched 110 of the 130 fields and so was quietly checking a subset. It now lives
        // in SpaFieldDocTests, which asks the type itself.
