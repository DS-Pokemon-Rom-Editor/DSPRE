...
namespace DSPRE.Tests
{
    ...
    ///
    /// FormProb[0] is Shellos and FormProb[1] is Gastrodon. The file stores a chance out of a hundred
    /// but encount_set.c:2578 only asks whether it is zero: zero is the west sea form, anything else is
    /// the east. AnnoonTable picks which Unown letters turn up, and zero means none at all
    /// (encount_set.c:2691 only reads it when it is non-zero, then subtracts one).
    /// </summary>
    public class WildFormDataTests
    {
        ...
        {
            var vm = new DSPRE.Avalonia.ViewModels.WildEditorDPPtViewModel();
