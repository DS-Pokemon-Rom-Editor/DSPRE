...
{
    /// <summary>
    /// The whole building-animation set, checked across every model and every map rather than by looking at
    /// one town.
    /// </summary>
    [Collection("rom")]
    public class BuildingAnimationSweepTests
    {
        ...
        public void EveryMapResolvesItsAnimationsAtEveryTimeOfDay()
        {
            ...
            // The two test targets run side by side against the same unpacked project, so on the first run
            // after a build one of them can be part way through unpacking the maps while this one starts
            // counting.
