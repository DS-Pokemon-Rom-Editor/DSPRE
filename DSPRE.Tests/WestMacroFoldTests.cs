...
{
    /// <summary>
    /// Folding a script into the shorthands it was written in, and back again.
    ///
    /// The point of folding is that a script reads the way somebody wrote it instead of the way it was
    /// assembled. The danger is that it changes the bytes, so this folds and unfolds every script in both
    /// games and requires the result to be identical, word for word. Doing that across all 1,002 scripts
    /// is what makes it meaningful: a fold that is wrong for one particular set of settings would pass on
    /// any single script.
    /// </summary>
    [Collection("rom")]
