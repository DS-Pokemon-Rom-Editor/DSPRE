...
{
    /// <summary>
    /// Every Pokemon's battle sprites are drawn in that Pokemon's own colours.
    ///
    /// All four sprites of a Pokemon share one set of colours; the only other set is the shiny one.
    /// Finding the colours by looking for the nearest palette in the archive instead picked the PREVIOUS
    /// Pokemon's SHINY colours for both back sprites, because those sit two files earlier and the rule
    /// preferred what came before. Neighbouring Pokemon are often close in colour, so it looked right.
    /// </summary>
    [Collection("rom")]
    public class BattleSpritePaletteTests
    {
        ...
        /// <summary>Asking for the shiny colours really changes the picture.
        ///
        /// A shiny Pokemon is the same drawing with a different set of colours, so the only thing that
        /// proves the shiny view works is that the pixels come out different. Checking that the right
        /// palette number was chosen would pass even if nothing used it.</summary>
        [Theory]
        [MemberData(nameof(Games))]
        public void AskingForShinyChangesThePicture(string code, string path, string game)
        {
            ...
            foreach (var a in GraphicAssets.All.Where(x => x.ShinyColourEntry != null))
            {
                ...
                // colours are the same as each other, so asking for shiny changes nothing. Those are named
                // rather than the check being loosened until everything passes.
                Assert.All(unchangedAt, i => Assert.InRange(i, 0, 3));
                Assert.True(differed + unchangedAt.Count == looked);
            }
        }

        /// <summary>The rule that used to be used, applied to the same archive, to show what it was doing
        /// and that this check can tell the difference. Guessing has to get the back sprites wrong, or
        /// declaring the answer was not worth doing.</summary>
        [Theory]
        [MemberData(nameof(Games))]
        public void GuessingTheNearestPaletteGetsTheBackSpritesWrong(string code, string path, string game)
        {
            ...
            // Guessing gets every back sprite wrong but the very first Pokemon's two, which have no
            // earlier palette to be misled by. Front sprites happen to land on the right one, which is
            // why half of this was right and half was quietly wrong.
