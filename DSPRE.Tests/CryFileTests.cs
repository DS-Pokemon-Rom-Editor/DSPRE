...
{
    /// <summary>
    /// Taking a cry out of a ROM and putting one back.
    ///
    /// What comes out is the sample on its own, not the sound the game makes with it. What goes back in
    /// is squeezed to four bits a sample, the way the games keep their own cries and the way hg-engine
    /// puts new ones in, so replacing a cry leaves the sound file about the same size: measured against
    /// HeartGold, re-importing Pikachu's own cry grew the archive by nothing at all.
    ///
    /// Squeezing is not exact. Across five real cries the error came to between three and six percent of
    /// the loudest point, which is the ordinary cost of four-bit sound.
    /// </summary>
