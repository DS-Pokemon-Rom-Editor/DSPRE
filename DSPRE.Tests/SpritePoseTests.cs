...
{
    /// <summary>
    /// Picking the walking picture out of a sprite bank.
    ///
    /// The layout was read off every readable bank in HeartGold: a bank keeps a group of pictures for
    /// each way of facing in the order up, down, left, right. People get four to a group, where the
    /// first and third are the same standing picture and the second and fourth are the two steps.
    /// Following Pokemon get two. The pace comes from fieldobj_drawdata.c, where Normal0 (line 1473)
    /// runs sixteen looping steps over four pictures and PairPoke (line 1584) twenty over two.
    /// </summary>
    public class SpritePoseTests
    {
        ...
        {
            // A step is eight frames, so the first tile shows the standing picture then the first step,
            // and the tile after shows the standing picture then the other step. That alternation is
            // the whole point: without it a walk looks like a slide.
