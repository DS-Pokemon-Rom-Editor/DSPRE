...
namespace DSPRE.Tests
{
    ...
    ///
    /// The type used to be read only for HeartGold and SoulSilver, because nobody had traced what Diamond,
    /// Pearl and Platinum do with it. Both games' field_3d_anime.c settles it:
    ///
    ///   CheckAddConditional  Platinum: type &amp; 0x01. HeartGold: the same bit, with the time-of-day type
    ///                        taken out first. So "waits to be set off" is true in both.
    ///   CheckSetConditional  Platinum: (type &gt;&gt; 1) &amp; 0x01. HeartGold: the same, and a time-of-day
    ///                        animation counts as one. Neither was read at all before.
    ///   TYPE_TIME_ANIME      0x8 in field_3d_anime_local.h, tested only in HeartGold's copy, with a
    ///                        comment saying it was added for Gold and Silver. Platinum has no such test,
    ///                        so a Platinum type of 8 does not mean this.
    ///
    /// The Door field is a different matter: the shorter record Diamond, Pearl and Platinum write does not
    /// contain it, so there is nothing to read rather than something untraced.
    /// </summary>
    public class BuildingAnimationTypeTests
    {
        ...
        /// two families must disagree about it. If they ever agree, one of them is being read wrongly.
        /// </summary>
        [Fact]
