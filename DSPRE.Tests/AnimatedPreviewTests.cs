...
{
    /// <summary>
    /// The animated preview, driven the way the window drives it but without a window. Uses the real
    /// HeartGold terrain animation, so it checks the whole chain: the area names an animation, the
    /// animation's material names match the scene's, and the frames it hands the renderer really move.
    /// Skipped when no local project is present.
    /// </summary>
    [Collection("rom")]
    public class AnimatedPreviewTests
    {
        ...
        /// <summary>
        /// The real HeartGold water animation. Says so plainly when the project is not on this machine,
        /// rather than handing back nothing and letting the test pass without having looked at anything.
        /// </summary>
        ...
        // The unit tests on OverworldAnimator prove it walks. These prove the preview draws it walking,
        // which is a separate thing and is where a wiring mistake would hide.
        ...
        /// <summary>
        /// There has to be a sprite before any of this is worth measuring, so no sprite is a failure
        /// rather than something to shrug off and carry on past.
        /// </summary>
        ...
        {
            // People are drawn with their walked offset added to where their feet were put, and their
            // feet come from the same place the player's do. If the two disagree about how big a tile is,
            // an overworld walks the wrong distance and drifts off the grid, which is what a scene that
            // has been shrunk to fit used to do.
