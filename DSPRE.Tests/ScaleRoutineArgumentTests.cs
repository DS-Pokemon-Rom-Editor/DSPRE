...
namespace DSPRE.Tests
{
    ...
    ///
    /// The game's own note calls that value "the scale at 100%" (wsp_tool.c:1713) and passes it straight in
    /// as the denominator (wsp_tool.c:1628). DSPRE reads the same slot but calls it a step and ignores it,
    /// dividing by a hard 100. That only matters if a move ever asks for something other than 100, so this
    /// reads every script in both games and says which values are actually used.
    /// </summary>
    [Collection("rom")]
