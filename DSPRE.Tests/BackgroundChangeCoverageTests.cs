...
{
    /// <summary>
    /// Every background a move asks for must be one the preview can actually draw.
    ///
    /// The player drops a background change on the floor when the image fails to build, with nothing said, so
    /// a move that blacks out the whole battlefield in the game just carries on looking normal here. Tri
    /// Attack, Extrasensory, Hydro Cannon and ExtremeSpeed all did.
    ///
    /// This reads every move script in the game rather than a few, collects the backgrounds they ask for, and
    /// requires each one to build.
    /// </summary>
    [Collection("rom")]
