...
{
    /// <summary>
    /// Where a script id really lives. The games do not keep every script in the map's own file: ids from
    /// 2000 up belong to shared files, one per kind, and the id is read relative to where that kind starts
    /// (scr_offset.h and SetScriptDataSub in script.c). Anything that runs a script has to resolve this
    /// first or it will look for the script in the wrong place.
    /// </summary>
