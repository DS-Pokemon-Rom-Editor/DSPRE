...
{
    /// <summary>
    /// No bare number is left on screen where the games have a name for it.
    ///
    /// This walks every parameter of every command in both games, asks whether anything can name that
    /// value, and where something can, requires the readable views to show the name or leave the setting
    /// out altogether. Leaving out a setting that is switched off is what keeps these lines short enough
    /// to read; showing its number instead of its name is the fault being looked for. The four sources of
    /// a name are the routine table, the target flags, the sound names and the settings lists in
    /// WestParamSchema; a parameter with none of those is a plain number and is left alone.
    ///
    /// It can fail because it looks the name up itself and then searches the rendered line for it, so a
    /// view that printed the number instead is caught. What it cannot say is whether the name is the
    /// RIGHT one; that comes from the per-word meanings and their citations.
    /// </summary>
    [Collection("rom")]
                ...
                foreach (var mode in new[] { WestViewMode.Guided, WestViewMode.Script })
                {
                    ...
                    foreach (var line in lines)
                    {
                        ...
                        for (int i = 0; i < c.Args.Length; i++)
                        {
                            ...
                            // switched off. Both are fine; what is not fine is the bare number. The
                            // labelled form is what makes that distinction checkable.
