...
{
    /// <summary>
    /// What a move animation is made of, named one mechanism at a time.
    ///
    /// This is the list a test set has to cover. It is derived from the script itself rather than written
    /// out by hand, so a mechanism cannot quietly appear in the games without turning up here: every
    /// opcode a script uses and every support routine it calls becomes a mechanism in its own right, on
    /// top of the broader groupings that matter for what gets drawn.
    /// </summary>
