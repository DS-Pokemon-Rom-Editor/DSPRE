...
namespace DSPRE.Tests
{
    ...
    ///
    /// The twenty style drawings all carry the same file tag, so nothing in the ROM itself says which is
    /// which; the order comes from winframe.naix in the leaked source. Painting style N with N marks and
    /// then stepping through the styles in the game's own Options menu shows whether stepping one style
    /// moves the mark count by one, which is the claim.
    ///
    /// A tool rather than a check, so it does nothing unless DSPRE_PAINT_ROM is set. The user's own
    /// project and ROM are never touched.
    /// </summary>
    [Collection("rom")]
    public class PaintedTextBoxRomBuilder
    {
        ...
        public void BuildARomWithEveryTextBoxStyleMarked()
        {
            ...
            for (int style = 0; style < Styles; style++)
            {
                ...
                // style number, in the colour the frame uses least so it cannot be mistaken for the
                // border itself. Style 0 gets one mark, so an unmarked box means something went wrong
                // rather than meaning style 0.
