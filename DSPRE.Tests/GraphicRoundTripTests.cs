...
namespace DSPRE.Tests
{
    ...
    ///
    /// Saving a picture is only useful if it can go back in, and the way to know that is to send one
    /// round the loop and compare the pixels. This works on a COPY of the project, so the real one is
    /// never written to.
    /// </summary>
    [Collection("rom")]
    public class GraphicRoundTripTests : IDisposable
    {
        ...
        ///
        /// These entries have their pixels run through a rolling key, so reading them straight gives
        /// coloured static that still passes every shape and round-trip check: a wrong reading written
        /// back wrong gives the same bytes. The thing that separates a sprite from noise is that a drawn
        /// sprite uses a handful of its sixteen colours in runs, while noise uses all of them evenly and
        /// almost never repeats. So that is what is measured here, over the whole archive rather than one
        /// of them, and the test is worth nothing unless it can fail: the same measurement is taken with
        /// the unscrambling turned off, and that reading has to be rejected.
        /// </summary>
        ...
        public void EveryBattleSpriteReadsAsAPictureAndNotAsNoise(string code, string path, string game, int dummies)
        {
            ...
            // Entries 0 to 3 are the slot for Pokemon number 0, which does not exist. Those four hold
            // filler that does not decode to a picture under the game's own key, checked separately with
            // a reader written from scratch: 6 percent of their pixels repeat a neighbour where a real
            // sprite is between 78 and 90 percent. They are never drawn, so they are allowed and named
            // rather than the measurement being loosened until everything passes.
        ...
        public void APaintedChangeSurvivesBeingSavedAndReopened()
        {
            ...
            foreach (var a in GraphicAssets.All)
            {
                ...
                // Start on the second row: the scrambled battle sprites cannot carry the first four
                // pixels, which is its own test below rather than something to trip over here.
