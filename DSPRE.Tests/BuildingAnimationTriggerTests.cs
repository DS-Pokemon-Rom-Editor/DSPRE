...
{
    /// <summary>
    /// Which building animations run by themselves and which wait to be set off. The engine reads the
    /// bottom bit of Type (CheckAddConditional in field_3d_anime.c) and registers the conditional ones
    /// stopped, for an event to start; doors are all of that kind, opened when you walk through them.
