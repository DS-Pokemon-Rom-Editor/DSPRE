...
{
    /// <summary>
    /// A sequence saved as a MIDI holds the same notes the sequence holds.
    ///
    /// Checked by reading the file back with a parser written for the purpose rather than by trusting the
    /// writer: a writer checked against itself proves nothing. Every note that went in has to come out at
    /// the same moment and the same pitch, and this is swept over every sequence in the archive rather
    /// than one of them.
    /// </summary>
    [Collection("rom")]
    public class MidiExportTests
    {
        ...
        public void EverySequenceSavesAsAMidiHoldingTheSameNotes(string code, string path, string game)
        {
            ...
            for (int i = 0; i < sdat.Sequences.Count; i++)
            {
                ...
                // Compare on the same footing the file uses. A MIDI counts in ticks, and several notes
                // written microseconds apart land on one tick, so sorting the expected list by its exact
                // seconds put them in a different order than the file has them in and looked like a fault.
                // Rounding both to ticks first is the honest comparison.
                var wanted = notes
        ...
        /// <summary>A saved MIDI holds the whole tune rather than the few seconds the preview plays.
        ///
        /// The preview stops early on purpose, and reusing its reading for the file quietly cut every
        /// tune off at eight seconds. This checks that reading further really does find more.</summary>
        [Theory]
        [MemberData(nameof(Games))]
