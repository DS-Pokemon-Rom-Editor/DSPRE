...
{
    /// <summary>
    /// Whether calling a routine actually makes the preview do anything.
    ///
    /// Reading the C says what each routine is supposed to do; this drives our own player with the words
    /// a real script passes and watches which of its outputs move. A routine that changes nothing at all
    /// is a routine the preview silently ignores, and that is the fault this is built to catch. It can
    /// detect it because every visible thing the player can do is one of the channels watched below: the
    /// two Pokemon's position, size, turn, tint, transparency, clip, mosaic and visibility, the screen
    /// shake, the background fade and flash, greyscale, the raster ripple, whether a background is up,
    /// how many particle emitters or cell actors exist, and whether a note was raised.
    ///
    /// What it cannot detect is a routine doing the WRONG thing in the right channel. That needs the
    /// per-word meanings in MoveAnimationRoutines.md, which is a separate check.
    /// </summary>
    [Collection("rom")]
    public class WestRoutineEffectTests
    {
        ...
        public void EveryRoutineTheScriptsCallMakesThePreviewDoSomething()
        {
            ...
            foreach (var kv in calls.OrderBy(k => k.Key))
            {
                ...
                // Watch every frame, not just the last one. Nearly everything here is transient: a shake
                // ends back where it started and a squash returns to full size, so comparing only the end
                // would call a working routine silent.
                bool moved = false;
            ...
            var expected = new[]
            {
                ...
                "ALL_DROP (78)",
                // Right to do nothing with the words the scripts actually pass. Putting a Pokemon back
                // where it belongs moves nothing when it is already there, and the one call of the shake
                // passes a single word, leaving the distance and the count as zeros.
                "WE_DISP_DEF (62)", "WE_175 / SHAKE (27)",
                // These act on something an earlier command in the real script creates: a particle
                // emitter, a dropped copy, or a cell actor. Driving one command on its own gives them
                // nothing to act on, so this harness cannot exercise them and says so rather than
