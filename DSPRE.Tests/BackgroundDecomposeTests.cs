...
namespace DSPRE.Tests
{
    ...
    ///
    /// A background is a sheet of eight by eight tiles, an arrangement saying which tile goes where and
    /// which way round, and the colours. The browser shows the three put together; this is the way back.
    ///
    /// What has to hold is that taking a picture apart and drawing it again gives the same picture. And
    /// backgrounds reuse tiles, so painting one square changes every square drawn from the same one: that
    /// is counted rather than hidden.
    ///
    /// Every test here puts the archive back as it found it.
    /// </summary>
    [Collection("rom")]
    public class BackgroundDecomposeTests
    {
        ...
        /// <summary>
        /// The check above proves able to fail: painting really does change the picture. It also shows
        /// what sharing a tile does, which is the thing about backgrounds people have to know.
        /// </summary>
        [Fact]
        public void PaintingABackgroundChangesItAndSaysWhatSharesATile()
        {
            ...
            foreach (int at in Drawings(a, 25))
            {
                ...
                // over the other. Taking the colour from the same square is what makes it reachable: each
                // square is drawn from one bank of sixteen colours, so a colour borrowed from elsewhere in
                // the picture may not exist in this square's bank at all.
