...
namespace DSPRE.Tests
{
    ...
    ///
    /// This is the check the round-trip tests cannot be: a battle sprite's pixels are scrambled with a key
    /// seeded from the file's own first two bytes, so reading it wrong and writing it back wrong gives
    /// identical bytes and every test passes. The only thing that separates a correct write from a
    /// convincing one is what the game draws.
    ///
    /// A mark is painted across the rival Chimchar's front sprite, which is what the savestates in
    /// ROMs\NDS\Plat\battle_test_saves put on screen as the enemy. The mark is made of colours the sprite
    /// already has, and the palette is not touched, so anything that shows is the pixel write and nothing
    /// else. This is a tool rather than a check, so it does nothing unless DSPRE_PAINT_ROM is set.
    /// The user's own project and ROM are never touched.
    /// </summary>
    [Collection("rom")]
