...
{
    /// <summary>
    /// How fast things happen, and where they are while they happen. The field runs at thirty frames a
    /// second and a walking step covers one tile in eight of them, so nothing ever jumps between tiles.
    /// </summary>
    public class FieldTimingTests
    {
        ...
        {
            // From the live table in field_camera.dat, entry 0. The copy of this table inside
            // field_camera.c is switched off behind an "#if 0" and gives a different angle, so a pitch
            // of about 45 degrees here would mean the dead one had been read by mistake.
            Assert.InRange(FieldCamera.DistanceInTiles, 41.5f, 42f);
            Assert.InRange(FieldCamera.PitchDegrees, 48.5f, 48.9f);

            // PerspWay is half the view angle, not all of it. GFC_SetCameraView builds the flat view's
            // box as tan(PerspWay) * Distance and uses it for top and bottom alike, which only matches
            // the perspective view if it is the half angle. Reading it as the whole angle would halve
            // what the camera sees and make everything look twice as far away.
        ...
        public void EveryCameraInTheGamesTableIsReadableAndSane()
        {
            ...
            foreach (var c in FieldCamera.Entries)
            {
                ...
                // Every row frames roughly the same amount of map, between about ten and twenty tiles,
                // even though the distances range from 20 tiles to nearly 100. That only comes out that
                // way if PerspWay is being read as the half angle.
