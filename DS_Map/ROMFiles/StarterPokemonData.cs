...
{
    // Reads and writes the starter Pokemon (Turtwig/Chimchar/Piplup on DP/Pt, Chikorita/Cyndaquil/Totodile
    ...
    // them straight into compiled ARM9 code. Ported from Universal Pokemon Randomizer FVX's
    // Gen4RomHandler.getStarters/setStarters (byte offsets/patterns verified against its gen4_offsets.ini
    // and Gen4Constants.java), with two changes: dialogue text is patched with a surgical old-name/old-type
    // to new-name/new-type substring swap instead of a hardcoded English rewrite, so localized ROMs keep
    // their own language, and the rival/tag-battle script search patterns are built from the current
    // starter species rather than a hardcoded vanilla ID, so editing starters more than once in the same
    // project keeps working instead of silently no-oping after the first edit.
    public static class StarterPokemonData
    {
        // Species bytes are placeholders, always rebuilt from the CURRENT starters via
        ...
        // Returns 0 (no held item) if this ROM's held-item location isn't known.
        public static int GetHeldItem()
        {
            if (RomInfo.gameFamily == RomInfo.GameFamilies.HGSS)
            {
                byte[] arm9 = ReadArm9();
                if (!TryFindHgssHeldItemOffset(arm9, out int offset)) return 0;
                return arm9[offset];
            }
        ...
        // No-op if this ROM's held-item location isn't known. The HGSS byte is 1 byte wide (item IDs
        // above 255 aren't representable there), DP/Pt's script-based field is a full word.
        public static void SetHeldItem(int itemId)
        {
            if (RomInfo.gameFamily == RomInfo.GameFamilies.HGSS)
            {
                byte[] arm9 = ReadArm9();
                if (!TryFindHgssHeldItemOffset(arm9, out int offset)) return;
                ARM9.WriteBytes(new[] { (byte)itemId }, (uint)offset);
                return;
            }
        ...
        // HGSS only, DP/Pt has no known equivalent offset. Returns the vanilla level (5) if this ROM's
        // offset isn't known.
        public static int GetStarterLevel()
        {
            if (RomInfo.gameFamily != RomInfo.GameFamilies.HGSS) return 5;
            byte[] arm9 = ReadArm9();
            if (!TryFindHgssLevelOffset(arm9, out int offset)) return 5;
            return arm9[offset];
        }

        // No-op on DP/Pt or if this ROM's offset isn't known.
        public static void SetStarterLevel(int level)
        {
            if (RomInfo.gameFamily != RomInfo.GameFamilies.HGSS) return;
            byte[] arm9 = ReadArm9();
            if (!TryFindHgssLevelOffset(arm9, out int offset)) return;
            ARM9.WriteBytes(new[] { (byte)level }, (uint)offset);
        }

        private static byte[] ReadArm9()
        {
            if (ARM9.CheckCompressionMark()) ARM9.Decompress(RomInfo.arm9Path);
            return ARM9.ReadBytes(0);
        }

        // Tries the vanilla fixed offset first, falls back to a full-ARM9 search only if what's there
        // doesn't match. The search needle excludes the item-ID byte itself since a modified ROM may
        // already hold something other than the vanilla item there.
        private static bool TryFindHgssHeldItemOffset(byte[] arm9, out int offset)
        {
            offset = -1;
            byte[] suffix = RomInfo.starterHeldItemArm9SearchSuffix;
            if (suffix == null) return false;

            uint fixedOffset = RomInfo.starterHeldItemOffsetHGSS;
            if (fixedOffset > 0 && MatchesAt(arm9, (int)fixedOffset + 1, suffix))
            {
                offset = (int)fixedOffset;
                return true;
            }

            var matches = DSUtils.SearchBytes(arm9, suffix);
            if (matches.Count != 1) return false;
            offset = matches[0] - 1;
            return true;
        }

        // Derived from the held-item offset (fixed distance, same subroutine), validated before trusting it.
        private static bool TryFindHgssLevelOffset(byte[] arm9, out int offset)
        {
            offset = -1;
            if (!TryFindHgssHeldItemOffset(arm9, out int heldItemOffset)) return false;

            int candidate = heldItemOffset + RomInfo.starterLevelOffsetFromHeldItem;
            byte[] validation = RomInfo.starterLevelValidationSuffix;
            if (candidate < 0 || validation == null || !MatchesAt(arm9, candidate + 1, validation)) return false;
        ...
        // itself couldn't be safely located/written, see GetStarters's HGSS ARM9-signature guard.
        public static bool ApplyStarters(int[] newSpecies)
        {
            ...
            RunBestEffort("rival/tag-battle scripts", () => PatchRivalAndTagBattleScripts(oldSpecies, newSpecies));
        ...
        // of UPR-FVX's Gen4RomHandler.setStarters DP/Pt graphics block, without this the minigame keeps
        // rendering the vanilla starter model even though the player ends up receiving the right species.
        ...
        private static void PatchRivalAndTagBattleScripts(int[] oldSpecies, int[] newSpecies)
        {
            if (RomInfo.gameFamily == RomInfo.GameFamilies.HGSS)
            {
        ...
        // name) for the new ones in the professor/rival dialogue, never a hardcoded English rewrite, so
